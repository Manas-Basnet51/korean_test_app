...
class TestsCubit extends Cubit<TestsState> {
  ...
  void searchTests(String query) {
    ...
    if (trimmedQuery.length < 2) {
      dev.log('Search query too short, restoring current category tests');
      _lastSearchQuery = '';
      if (_currentCategory == TestCategory.all) {
        loadInitialTests();
      } else {
  ...
  Future<void> createTest(TestItem test) async {
    ...
    try {
      ...
      _handleError('Failed to create test: $e', TestsOperationType.createTest);
    }
  }
  ...
  Future<void> updateTest(String testId, TestItem updatedTest) async {
    ...
    try {
      emit(state.copyWith(
        currentOperation: TestsOperation(
          type: TestsOperationType.updateTest,
          status: TestsOperationStatus.inProgress,
          testId: testId,
        ),
      ));
      ...
      result.fold(
        onSuccess: (success) {
          if (success) {
            ...
            dev.log('Test updated successfully: ${updatedTest.title}');
          } else {
            emit(state.copyWithBaseState(
      ...
      _handleError('Failed to update test: $e', TestsOperationType.updateTest, testId);
    }
  }
  ...
  Future<void> deleteTest(String testId) async {
    ...
    try {
      emit(state.copyWith(
        currentOperation: TestsOperation(
          type: TestsOperationType.deleteTest,
          status: TestsOperationStatus.inProgress,
          testId: testId,
        ),
      ));
      ...
      result.fold(
        onSuccess: (success) {
          if (success) {
            ...
            dev.log('Test deleted successfully: $testId');
          } else {
            emit(state.copyWithBaseState(
      ...
      _handleError('Failed to delete test: $e', TestsOperationType.deleteTest, testId);
    }
  }
  ...
  Future<bool> uploadNewTest(TestItem test) async {
    try {
      ...
      if (currentState.status == TestsOperationStatus.completed && 
          currentState.type == TestsOperationType.createTest) {
        return true;
      }
  ...
  Future<bool> updateExistingTest(String testId, TestItem updatedTest) async {
    try {
      ...
      final currentState = state.currentOperation;
      if (currentState.status == TestsOperationStatus.completed && 
          currentState.type == TestsOperationType.updateTest) {
        return true;
      }
  ...
  Future<bool> deleteExistingTest(String testId) async {
    try {
      ...
      final currentState = state.currentOperation;
      if (currentState.status == TestsOperationStatus.completed && 
          currentState.type == TestsOperationType.deleteTest) {
        return true;
      }
  ...
  Future<List<TestResult>> getTestResultsWithHandling() async {
    try {
      final results = await getUserTestResults();
  ...
  void clearSelectedTest() {
    emit(state.copyWith(selectedTest: null));
  }
